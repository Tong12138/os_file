...
bool create(string file_name)         // 创建文件
{
	...
	if (language) cout << "该文件被分配的数据块号是: " << temp.beginning_in_dataArea << endl;
...
bool close(string file_name)
{
	...
	if (pos == -1)
	{
		if (language)cout << "当前目录不存在 " << file_name << " 文件" << endl;
	...
	if (language) cout << "关闭文件 " << file_name << " 成功" << endl;
...
bool delfile(string file_name) //删除文件
{
	...
	if (language) cout << "删除文件 " << file_name << " 成功" << endl;
...
bool open(string file_name)
{
	...
	{
		if (language)cout << "内存空间不足，无法读入 " << file_name << " 内容,请关闭无用文件" << endl;
